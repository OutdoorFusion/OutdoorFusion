...
noise_revenue = np.random.normal(scale=1000, size=len(predicted_revenue))
noise_profit = np.random.normal(scale=100, size=len(predicted_profit))
...
output_data.to_csv('combined_revenue_profit_product.csvOUD', index=False)